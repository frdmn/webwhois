...
var evaluteWhoisResponse = function(error, data){
  if (error) {
    ...
    if (error.code === 'ECONNRESET') {
      return 'Can\'t establish connection to AutoDNS whois server. IP noch whitelisted?';
    } else {
...
var whoisDomain = function (domain, callback){
  ...
  whois.lookup('full ' + domain, options, function(err, data) {
    var evaluateError = evaluteWhoisResponse(err, data);
...
var checkAvailability = function (domains, cb){
  ...
  whois.lookup('multi ' + domainString, options, function(error, whoisResponse) {
    ...
    var evaluateError = evaluteWhoisResponse(error, whoisResponse);
    if (evaluateError !== true) {
      responseObject.status = 'error';
      responseObject.message = evaluateError;
